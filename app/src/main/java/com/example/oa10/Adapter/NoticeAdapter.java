...
public class NoticeAdapter extends BaseAdapter {
    private List<Notice.NoticeBean> list;
    private Context context;

    //通过构造方法接受要显示的新闻数据集合
    public NoticeAdapter(Context context, List<Notice.NoticeBean> list) {
        this.list = list;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Notice.NoticeBean noticeBean = list.get(position);
        //4.将数据设置给这些子控件做显示
        //item_img_icon.setImageDrawable(newsBean.icon);//设置imageView的图片
        item_tv_title.setText(noticeBean.getNotice_title());
        item_tv_des.setText(noticeBean.getNotice_time());
